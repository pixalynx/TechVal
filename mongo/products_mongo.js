...
class MongoDatabase {
    ...
    async getAllProducts(){
        try{
            ...
            let result = await db.db(this.database).collection(Products).find({}).toArray();
            return result;
            db.close();
        }catch(err){
            let result = false;
    ...
    async getProductbySerial(serialNumber){
        ...
        try{
            ...
            let db = await MongoClient.connect(MongoConn);
            let result = await db.db(this.database).collection(Products).find(query).toArray();
            if(result.length){
                return result
                db.close();
            }else{
    ...
    async createProduct(productObject){
        ...
        try{
            let db = await MongoClient.connect(MongoConn);
            let insert = await db.db(this.database).collection(Products).insertOne(this.productObject);
            return true
            db.close();                        
        }catch(err){
            return false;
