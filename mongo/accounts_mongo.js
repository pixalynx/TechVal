...
class MongoDatabase {
    ...
    async getAllAccounts(){
        try{
            ...
            let result = await db.db(this.database).collection(Accounts).find({}).toArray();
    ...
    async getAccountByName(accountName){
        ...
        try{
            ...
            let db = await MongoClient.connect(MongoConn);
            let result = await db.db(this.database).collection(Accounts).find(query).toArray();
            if(result.length){
                return result
                db.close();
            }else{
    ...
    async createAccount(accObj){
        ...
        try{
            let db = await MongoClient.connect(MongoConn);
            let insert = await db.db(this.database).collection(Accounts).insertOne(this.accObj);
            return true;
            db.close();
        }catch(err){
            return false;
